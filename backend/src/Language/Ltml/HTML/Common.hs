...
anchorLink label = a_ [cssClass_ Class.AnchorLink, href_ (cons '#' $ unLabel label), onclick_ scrollScript]
  where
    scrollScript = "event.preventDefault(); document.getElementById('" <> unLabel label <> "').scrollIntoView({behavior: 'smooth'});"
